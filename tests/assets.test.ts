...
import { createAssetRecord, listAssets, publicAssetUrl, toPublicAssetRecord, upsertGeneratedAssetPaths } from "@/lib/storage/assets";
...
describe("asset records", () => {
  it("creates local asset metadata and preview URLs", () => {
    const asset = createAssetRecord({
      ...
      mimeType: "image/png",
      size: 1234,
      generationBatchId: "batch-upload-1"
    });

    expect(asset.kind).toBe("upload");
    expect(asset.generationBatchId).toBe("batch-upload-1");
    expect(asset.name).toContain("product");
    expect(publicAssetUrl(asset)).toBe(`/api/assets/file/${asset.id}`);
    expect(toPublicAssetRecord(asset)).toMatchObject({
      id: asset.id,
      generationBatchId: "batch-upload-1",
  ...
  it("upserts generated workflow images into the asset library by path", async () => {
    ...
    const first = await upsertGeneratedAssetPaths([{ path: imagePath }], { prompt: "prompt", generationBatchId: "batch-workflow-1" });
    ...
    expect(assets[0].generationBatchId).toBe("batch-workflow-1");
  });
});
